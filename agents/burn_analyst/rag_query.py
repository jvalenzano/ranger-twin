"""
Burn Severity Knowledge Base Tool (Vertex AI RAG).

Provides RAG capabilities for burn severity assessment, MTBS protocols,
dNBR analysis, BAER guidelines, and soil burn severity standards.

Usage:
    from rag_query import query_burn_severity_knowledge

    result = query_burn_severity_knowledge(
        query="What are the dNBR thresholds for high severity burn classification?",
        top_k=5
    )
"""
...
from agents.burn_analyst.skills.template_lookup import lookup_template
...
def query_burn_severity_knowledge(
    query: str,
    top_k: int = 5,
    include_answer: bool = True
) -> dict[str, Any]:
    """
    Query burn severity knowledge base using Vertex AI RAG.

    Retrieves technical guidance on MTBS classification, dNBR analysis,
    BAER assessment protocols, soil burn severity, and debris flow risk.

    **ADK Compatible**: Uses only simple parameter types (str, int, bool).

    Args:
        query: Natural language query about burn severity assessment
    ...
    Example:
        >>> result = query_burn_severity_knowledge(
        ...     "What are the BAER protocols for high severity burns?"
        ... )
        >>> print(result["answer"])
    """
    try:
        ...
        if not is_rag_available():
            ...
            logger.info("[RAG-OFFLINE] Defaulting to embedded templates for Burn Analyst")
            ...
            return {
                ...
                "reasoning_chain": [
                    ...
                    "Retrieved MTBS/BAER severity standards from embedded knowledge"
                ],
                "confidence": 0.85,
                "status": "success",
